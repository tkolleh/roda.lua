...
function argp:parse_comma_values(values_str, opt)
	...
	for part in values_str:gmatch("[^,]+") do
		...
		if convertedValue then
			...
			error(
				("%s: option ‘--%s’: %s value expected, got “%s”"):format(self.name, opt.dest, opt.type, part)
			)
		end
...
function argp:parse_short_or_long(arg, type)
	...
	elseif opt.count_params == 1 then
		if value ~= nil and value == "" then
			-- Return nil so parse() can look ahead for the next argument
			return opt.dest, nil
		end
		...
		if convertedValue == nil then
			error(
				string.format("%s: option '--%s': %s value expected, got \"%s\"", self.name, opt.dest, opt.type, value)
			)
		end
	...
	while i <= #args do
		...
		if current:find("^%-%-") then
			...
			if value == nil or (type(value) == "string" and value == "") then
				local opt = self._options_map[current:match("^%-%-(.+)")]
				if opt and opt.count_params >= 1 and i + 1 <= #args and not args[i + 1]:find("^%-") then
					i = i + 1
					value = typeConverter(opt.type, args[i])
				end
			end
			result[dest] = value
